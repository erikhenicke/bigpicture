...
def resolve_region(region) -> tuple[int | None, str]:
    """Resolve a region given as an int (0-5) or name to (index, label).

    Returns ``(None, 'all')`` when ``region`` is None (no region filter).
    """
    if region is None:
        return None, "all"
    try:
...
def load_metadata(
    ...
    """Load the extended metadata, assign WILDS splits, filter, drop rows w/o span.

    Parameters
    ----------
    metadata_path : path to ``rgb_metadata_extended.csv``.
    split : 'all' (WILDS union, default) or one of ``WILDS_SPLITS``.
    region : None (all regions), or a region index 0-5 / name (see ``REGION_NAMES``).
    """
...
def class_extent_stats(
    ...
    """Return spatial-extent statistics (km) for a class and for all classes pooled.

    Parameters
    ----------
    label : integer class label in [0, 61].
    metadata_path : path to the extended metadata CSV (ignored if ``df`` given).
    split : 'all' (WILDS union) or one of ``WILDS_SPLITS``.
    region : None (all regions) or a region index 0-5 / name.
    df : optional pre-loaded frame from :func:`load_metadata` (avoids re-reading).

    Returns
    -------
    dict with keys: label, category, split, region, n,
    extent_{mean,median,min,max,std}_km, and the corresponding overall_* keys
    (plus n_total) for all classes pooled.
    """
...
def all_class_stats(
    ...
    """Per-class extent stats (mean/median/min/max/std, km) as a DataFrame."""
    df = load_metadata(metadata_path, split=split, region=region)
